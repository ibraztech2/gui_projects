def add_date():
    def spinbox_window_terminator():
        date_created = datetime_stamp
        completion_date = f"{get_completion_year.get()}-{get_completion_month.get()}-{get_completion_day.get()}"
        task_list.append(f"{task_entry.get()}~{task_status}~{date_created}~{completion_date}")
        save_task()
        display_task()
        spin_box_window.destroy()

    get_completion_day = IntVar()
    get_completion_month = IntVar()
    get_completion_year = IntVar()
    task_status = False
    if task_entry.get():
        spin_box_window = Toplevel(todo_list_frame_1)
        Spinbox(spin_box_window, from_=1, increment=1, to=30, textvariable=get_completion_day).grid()
        Spinbox(spin_box_window, from_=1, increment=1, to=12, textvariable=get_completion_month).grid()
        Spinbox(spin_box_window, from_=datetime_stamp.year, increment=1, to=datetime_stamp.year + 50,
                textvariable=get_completion_year).grid()
        Button(spin_box_window, text="Done", command=spinbox_window_terminator).grid()

        spin_box_window.grid()
    else:
        messagebox.showerror("Date Error","Input task before adding date")
...
def check_task():
    ...
    if get_current_selection:
        ...
        task_list[get_current_selection] = f"{current_task}~{new_status}~{date_created}~{completion_date}"
        save_task()
        display_task()
    else:
...
def task_details():
    ...
    if get_current_selection:
        ...
        if get_task_details_val:
            current_task, status, date_created, completion_date = task_list[get_current_selection].split("~")
            year, month, date = completion_date.split("-")
...
create_button = Button(button_frame, text="Add Date", height=button_height, width=button_width, command=add_date,
                       justify="center")
...
exit_button = Button(button_frame, text="Exit", height=button_height, width=button_width, command=exit_todo,
                     justify="center")
task_details_button = Checkbutton(todo_list_frame_1, anchor="center", justify="center", height=button_height,
                                  width=button_width,
                                  text="Show Task Details", command=task_details, onvalue=True, offvalue=False,
                                  variable=task_detail_contl_var, state="active")
